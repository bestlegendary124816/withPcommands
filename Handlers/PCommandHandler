...
module.exports = (client) => {
    ...
    fs.readdirSync('./commands/prefix/').forEach(dir => {
        const commands = fs.readdirSync(`./PCommands/${dir}/`).filter(file => file.endsWith('.js')) // filter out all the files ends with .js

        for (let file of commands) {
            let command = require(`../PCommands/${dir}/${file}`); // require the command file
